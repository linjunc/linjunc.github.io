...
借助一些构建工具，例如 babel，本文就通过 Babel 来介绍，JSX 是如何转译成 JS 代码
...
那么 Babel 是如何工作的呢？下面先介绍一下 Babel 的工作流程
...
- 如果是经典（classic）的方式,也就是**手动引入 React 的方式**，那么就只需要生成 `createElement` 和 `fragment` 的 id
