...
:::tip
WebSocket 的握手过程使用的是 HTTP 协议，首先会像服务端发送一个 HTTP 请求，其中**包含了需要升级 WebSocket 请求的头部信息，Upgrade 和 Connection 等**，服务端接收到请求后，会验证请求是否合法，返回 HTTP 响应，其中包含了 WebSocket-Accept 的字段，用于确认服务器是否支持 WebSocket 协议，并对连接请求进行验证，验证通过，那么就会升级协议，之后的通信就是基于 WebSocket 协议了。
:::
